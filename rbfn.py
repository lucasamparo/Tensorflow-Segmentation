            ...
            for batch_i in range(dataset.num_batches_in_epoch()):
                ...
                if batch_num % 100 == 0 or batch_num == n_epochs * dataset.num_batches_in_epoch():
                    ...
                    '''if test_accuracy >= max_acc[0]:
                        ...
                        tf.train.write_graph(minimal_graph, '.', "save/rbfn/minimal_graph.txt", as_text=True)
                        saver = tf.train.Saver(tf.global_variables())
                        saver.save(sess,"save/rbfn/checkpoint.data", global_step=0)

                        checkpoint_state_name = "checkpoint_state"
                        input_graph_name = "input_graph.pb"
                        output_graph_name = "output_graph.pb"

                        input_graph_path = "save/rbfn/minimal_graph.txt"
                        input_saver_def_path = ""
                        input_binary = False
                        input_checkpoint_path = 'save/rbfn/checkpoint.data' + "-0"
